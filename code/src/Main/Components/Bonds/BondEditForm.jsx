...
const BondEditForm = (props) => {
  ...
  const [text, setText] = useState("Edit Bond");
  ...
  let changeData = (e, type) => {
    ...
    setData({
      bondTransactionID: data.bondTransactionID,
      type: "2",
  ...
  let handleSubmit = async (e) => {
    ...
    const EDIT_ENDPOINT = "http://localhost:80/evolve/addEditDelBond.php";
    //console.log(data);
  ...
  let op1 = ["Corporate", "Municipal", "Government", "Agency", "Other"];
  let op2 = ["Annual", "Semi Annual", "Quarterly", "Monthly"];
  ...
  return (
    <>
      <div id="box">
        ...
        <div id="form-holder">
          <span className="form-title">Edit Bond</span>
          {/*Vertical line*/}
          ...
          <div id="form-content" style={{ borderLeft: props.border }}>
            <form onSubmit={(e) => handleSubmit(e)}>
              ...
              <select className="selectbox" name="bondtype" defaultValue={data.bondtype} onChange={(e) => changeData(e, 1)} required>
              ...
              <select className="selectbox" name="paymentInterval" defaultValue={data.paymentInterval} onChange={(e) => changeData(e, 6)} required>
              ...
              {text === "Edit Bond" ? <button type="submit">{text}</button> : <span id="status">Editing...</span>}
            </form>
          </div>
        </div>
      </div>
